...
#import "badanieViewController.h"
#import "SWRevealViewController.h"
#import "AppDelegate.h"
#import "badanieTheSecondViewController.h"

@interface badanieViewController ()
{
...
@implementation badanieViewController
...
- (IBAction)sendButtonClicked:(id)sender
{
    ...
    {
        UIAlertView *przebiegAlert = [[UIAlertView alloc] initWithTitle:@"Podaj datę" message:nil delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [przebiegAlert show];
    }
    else
    {
        ...
        {
            UIAlertView *dataAlert = [[UIAlertView alloc] initWithTitle:@"Podaj wartość" message:nil delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [dataAlert show];
        }
        else
        {
            if (self.numerFakturyTextField.text.length == 0)
            {
                UIAlertView *wykonawcaAlert = [[UIAlertView alloc] initWithTitle:@"Podaj numer faktury" message:nil delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                [wykonawcaAlert show];
            }
            else
            {
...
-(void)network
{
    ...
    NSString *post = [NSString stringWithFormat:@"idKierowcy=%@&idPojazdu=%@&wartoscOplaty=%@&dataOplaty=%@&nrFaktury=%@&request=dodajOplate", idKierowcy,idPojazdu, self.wartoscTextField.text, self.dataTextField.text, self.numerFakturyTextField.text];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (idOplaty != nil) {
        [self performSegueWithIdentifier:@"badanie" sender:self];
    }
